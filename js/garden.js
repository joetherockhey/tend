...
const Garden = (function () {
  ...
  /* What actually stops you walking: the scenery the world put there, which
     you clear with an axe or a shovel rather than by picking it up.

     Anything you bought or could pick up does NOT stop you - you walk over
     your own axe, your own bench, your own everything, the same way you
     already walked over your own plants, logs and pets. Standing on a thing is
     also the first place Pick up looks, so walking onto it and taking it is
     one move. Placement still treats it as taken (see cellOccupied), so
     nothing ever ends up stacked two deep on one square. */
  function findBlockingSceneryAt(row, col) {
    const d = findDecorationAt(row, col);
    return d && !d.movable ? d : null;
  }
  ...
  function cellOccupied(row, col, ignoreHero) {
    ...
    /* The shop asks "where can a new seedling land", and the answer must not be
       under the gardener's feet. Setting something down is the opposite
       question - you are standing there on purpose - so it passes ignoreHero.
       Beds and other flat ground are not blocking and never count as occupied,
       which is what keeps planting in a bed possible. */
    if (!ignoreHero && row === heroPos.row && col === heroPos.col) return true;
    return false;
  }
  ...
  function dropOrStackLog() {
    ...
    /* Stacking onto a site or another log is the point, so those two cases are
       handled above. Setting a log down on fresh ground still needs the ground
       to be free of everything else. */
    const spot = dropSquare();
    if (!spot) { showThought('No room here'); return; }
    groundLogs.push({ id: heldLog.id, row: spot.row, col: spot.col });
    heldLog = null;
    saveGroundLogs();
    renderGarden();
  }
  ...
  function stepHero(dr, dc, opts) {
    ...
    /* Plants you bought are the one thing you walk over. Everything else stops
       you again: saplings and the trees they become, blocking scenery, and a
       finished cabin. Walking onto a plant waters it; bumping into a sapling
       waters that, which is what the wall was always carrying. */
    const blocked = () => { saveHeroPos(); positionHero(); return false; };

    const saplingHere = saplings.find(s => s.planted && s.row === targetRow && s.col === targetCol);
    if (saplingHere) {
      if (!isSaplingGrown(saplingHere) && mayWater) {
        waterSaplingCheck(targetRow, targetCol, saplingHere);
      }
      return blocked();
    }

    if (findBlockingSceneryAt(targetRow, targetCol)) return blocked();

    const cabinHere = cabinSites.find(s => s.row === targetRow && s.col === targetCol);
    if (cabinHere && cabinHere.complete) return blocked();

    const plantId = findPlantAt(targetRow, targetCol);
    if (plantId && mayWater) waterCheck(targetRow, targetCol, plantId);
    ...
    checkTreatDelivery();
    checkAnimalSounds();
    lensLook(heroPos.row, heroPos.col);
    return true;
  }
  ...
  /* Where a carried thing should be set down. You can stand on top of another
     plant now, so "drop it where I am" would quietly stack two things on one
     square - which is exactly the bug that used to lose seedlings bought in
     bulk. Under your feet if that square is free, otherwise the one you are
     facing, otherwise nowhere and you are told why. */
  function dropSquare() {
    if (!cellOccupied(heroPos.row, heroPos.col, true)) return { row: heroPos.row, col: heroPos.col };
    const [fr, fc] = facingSquare();
    if (fr >= 0 && fr <= gardenMaxUnlockedRow && fc >= 0 && fc < GARDEN_COLS && !cellOccupied(fr, fc)) {
  ...
  function togglePickup(opts) {
    ...
    if (heldSapling) {
      ...
      if (sapling) {
        sapling.row = spot.row;
        sapling.col = spot.col;
    ...
    if (heldDecoration) {
      ...
      if (heldDecoration.source === 'theme') {
        movableLayout[heldDecoration.instanceId] = { row: spot.row, col: spot.col };
        saveMovableLayout();
      } else if (heldDecoration.source === 'item') {
        const item = purchasedItems.find(p => p.id === heldDecoration.sourceId);
        if (item) { item.row = spot.row; item.col = spot.col; }
        savePurchasedItems();
      }
      heldDecoration = null;
      renderGarden();
    ...
    if (heldTreat) {
      ...
      if (idx !== -1) pendingTreats[idx] = { ...pendingTreats[idx], row: spot.row, col: spot.col };
      heldTreat = null;
      renderGarden();
      return;
    }
  ...
  function togglePickupRest() {
    ...
    /* Now that you can walk over things, the square you are standing on comes
       ...
       used to be impossible. Then the square you are facing, then the rest. */
    if (takeFromSquare(heroPos.row, heroPos.col)) return;
  ...
  /* The square you are on plus the four around it - whatever you have walked
     onto or up to. The nearest thing wins, and the same thing does not announce
     itself twice in a row. */
  function lensLook(row, col) {
  ...
  function helpTopics() {
    ...
    return {
      ...
      pickup: {
        ...
        body: 'You can walk over anything of your own. Stand on it, or next to it, and '
          + pick + '. To set it down again, ' + drop + ' on an empty square.'
      },
      axe: {
  ...
  /* These three lines only show inside the help sheet on a phone now - see
     .garden-hint in the stylesheet. */
  function useHintText() {
    const own = 'Walk over anything of your own. Only ' + terms().chopTarget
      + ' and the like stop you.';
    if (!phoneControls()) {
      return own + ' Press E to pick up whatever you are standing on or facing,'
        + ' and E again to put it down.';
    }
    return own + ' Pick up, Use and Put down are under the '
