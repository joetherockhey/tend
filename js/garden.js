...
const Garden = (function () {
  ...
  function stepHero(dr, dc, opts) {
    ...
    /* Nothing stops you any more. Trees, beds, tables, plants, the cabin - you
       walk over all of it. Working round the scenery a square at a time was the
       fiddliest part of the garden, and the game underneath is watering things
       and carrying them about, not navigating a maze.

       Walking onto a plant still waters it, which is the one thing the old wall
       was carrying: you had to bump into a plant to water it, and now you step
       onto it instead. */
    ...
    if (saplingHere && !isSaplingGrown(saplingHere) && mayWater) {
  ...
  function helpTopics() {
    ...
    return {
      ...
      pickup: { icon: '\u{270B}', title: 'Picking things up', body: (phoneControls() ? 'Walk onto a ' + t.plant + ', tool, ' + t.log + ' or ' + t.sprout + ' - nothing blocks you - and tap Pick up. Tap Put down to set it on a free square, or Use it if it is a tool. The box beside the buttons always shows what you are carrying.' : 'Walk onto a ' + t.plant + ', tool, ' + t.log + ' or ' + t.sprout + ' - nothing blocks you - and press E to pick it up. Standing on it counts first, then whatever you are facing. Press E again to put it down on a free square, or use it if it is a tool. The box beside the plot shows what you are carrying.') },
  ...
  function useHintText() {
    if (!phoneControls()) {
      return 'Walk anywhere - nothing is in your way. Press E to pick up whatever'
        + ' you are standing on or facing, and E again to put it down.';
    }
    return 'Walk anywhere - nothing is in your way. Pick up and Put down are under the '
