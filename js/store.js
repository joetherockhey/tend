...
const Store = (function () {
  ...
  /* Defaults are offered ONCE, to an account that has no categories at all.
     An account that has any is left alone for ever.

     There used to be a top-up beside this: an account created before a default
     existed had the new ones added, guarded by a DEFAULTS_VERSION marker in
     prefs so it only happened once. The marker was the whole of its memory,
     and prefs are replaced wholesale by a pull - so on any account whose
     server row did not carry the marker yet, every load added back every
     default the person had deleted. Delete Errands and Fun on the overview
     page, reload, and there they are again at the top of the list.

     The top-up is gone rather than repaired. A guard that has to remember
     something to avoid undoing a deliberate deletion will keep finding ways to
     forget; not re-adding anything cannot. An old account misses out on a
     default added later, which it can add itself in a second, and which is a
     far smaller thing than a category refusing to stay deleted. */
